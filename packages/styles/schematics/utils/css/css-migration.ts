...
export class CssMigration {
    ...
    currentElement (element: TmplAstElement | null) {
        this.filteredUpdates = this.updates
            ...
            .filter(u => u.class === null || element?.attributes.some(a => a.name === 'class' && a.value.split(' ').some(v => u.class?.test(v))))
            .filter(u => u.attribute === null || element?.attributes.some(a => u.attribute?.test(a.name)));
    }
