...
export class CssMigration {
    ...
    currentElement (element: TmplAstElement | null) {
        this.filteredUpdates = this.updates
            .filter(u => u.tag === null || (u.tag !== null && u.tag.test(element?.name ?? '')))
            .filter(u => u.class === null || element?.attributes.some(a => a.name === 'class' && a.value.split(' ').some(v => u.class !== null && u.class.test(v))))
            .filter(u => u.attribute === null || element?.attributes.some(a => u.attribute !== null && u.attribute.test(a.name)));
    }

    currentTemplate (element: TmplAstTemplate | null) {
        this.filteredUpdates = this.updates
            .filter(u => u.tag === null || (u.tag !== null && u.tag.test(element?.tagName ?? '')))
            .filter(u => u.class === null || element?.attributes.some(a => a.name === 'class' && a.value.split(' ').some(v => u.class !== null && u.class.test(v))))
            .filter(u => u.attribute === null || element?.attributes.some(a => u.attribute !== null && u.attribute.test(a.name)));
    }
