...
3. **Run migration schematics** to apply any code changes required by the new version:
   ...
   > **Note:** The `-r` flag runs the migration schematics recursively across **all** workspace packages that define a `ng` script in their `package.json`. If you only need to update this library, use:
   >
   > ```bash
