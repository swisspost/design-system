...
!function (factory) {
  ...
  vertx.EventBus = function (url, options) {
    ...
    that.login = function (username, password, replyHandler) {
      sendOrPub("send", 'vertx.basicauthmanager.login', {
        username: username,
        password: password
      }, function (reply) {
        if (reply.status === 'ok') {
...
(function ($) {
  window.klpWidgetDev = (function (id, app, service, appLoginURL, menuLinks, lang, platform, options, environment) {
      ...
      for (var mutation of mutationsList) {
        if (mutation.attributeName === "class") {
          ...
          if ($("header").hasClass('h-fixed-position') && !$("header").hasClass('h-visible')) {
            ...
            selectFromShadowDom().find("#" + id).removeClass("bubble");
          }
        }
      }
    };
    ...
    var keepAliveID = 'klp-widget-keepalive',
      ...
      texts = {
        de: {
          ...
          'change-company-confirm-dialog': 'M\u00F6chten Sie die aktuelle Firma wechseln? </p></div></div><div class="modal-button-container row"><div class="col-6"></div><div class="col-6 item-centered"><button id="klp-widget-authenticated-dochangecompany" class="ppm-button btn btn-primary col-11">Firma w\u00E4hlen</button>',
          'change-company-support-dialog': 'Im Kunden-Supportlogin steht die Funktion "Firma wechseln" nicht zur Verf\u00FCgung. F\u00FChren Sie stattdessen ein Logout durch und starten Sie das Kunden-Supportlogin auf einem neuen Benutzer oder einer neuen Firma. </p></div></div><div class="modal-button-container row"><div class="col-6"></div><div class="col-6 item-centered"><button id="klp-widget-authenticated-dochangecompany" class="ppm-button btn btn-primary col-11">Logout</button>',
        ...
        en: {
          ...
          'change-company-confirm-dialog': 'Would you like to sign-out from the current company? </p></div></div><div class="modal-button-container row"><div class="col-6"></div><div class="col-6 item-centered"><button id="klp-widget-authenticated-dochangecompany" class="ppm-button btn btn-primary col-11">Next</button>.',
          'change-company-support-dialog': 'On Kunden-Supportlogin, the function Change company is not available. You have to SignOut and then start again a new Kunden-Supportlogin selecting a different account or company. </p></div></div><div class="modal-button-container row"><div class="col-6"></div><div class="col-6 item-centered"><button id="klp-widget-authenticated-dochangecompany" class="ppm-button btn btn-primary col-11">Logout</button>',
          'change-account': 'Change user account',
          'change-account-confirm-dialog': 'Do you want to change to another user account? </p></div></div><div class="modal-button-container row"><div class="item-centered d-flex justify-content-center col-lg-12 col-12 pe-0 me-0"><div class="col-lg-4 col-12 me-0 pe-0"><button id="klp-widget-authenticated-dochangecompany" class="ppm-button btn btn-primary w-100">Next</button>',
          'change-account-support-dialog': 'The "Change user account" function is not available in the customer support login. Instead, log out and start the customer support login using a new user or new company. </p></div></div><div class="modal-button-container row"><div class="col-6"></div><div class="col-6 item-centered"><button id="klp-widget-authenticated-dochangecompany" class="ppm-button btn btn-primary col-11">Logout</button>',
    ...
    function logoutURL() {
      ...
      var inIframe = false;
      return join(platform.logoutURL, 'app=' + app + '&lang=' + currentLang + '&service=' + serviceForLogout + '&inIframe=' + inIframe + '&logoutTargetURL=' + conf.logoutTargetURL)
    }
    ...
    function setChangeAccountDialog() {
      selectFromShadowDom().find('#' + id + ' #klp-widget-authenticated-menu-changecompany').on('click touch', function (e) {
        e.preventDefault();
        selectFromShadowDom().find('#' + id + ' #klp-widget-authenticated-menu-changecompany').focus();
        changeAccountDialog();
        return false;
      })
    }
    ...
    function changeAccountDialog() {
      ...
      if (selectFromShadowDom().find('#' + id + ' #klp-widget-authenticated-changecompanydialog').length === 0) {
        var changecompanyDialog = '<div id="changeAccountModal" class="modal"><div class="modal-content"><div class="modal-text-container row"><div class="col-12 text-align-center"><i class="pi pi-2086"></i></div><span class="close">&times;</span><div class="col-1"></div><div class="col-10 text-align-center"><p class="modal-text">' + body + '</div></div></div>';
        selectFromShadowDom().find('#' + id + ' .klp-widget-authenticated').append(changecompanyDialog);
        ...
        selectFromShadowDom().find('#klp-widget-authenticated-dochangecompany').on('click touch', function (e) {
          e.preventDefault();
          selectFromShadowDom().find('#' + id + ' #klp-widget-authenticated-dochangecompany').focus();
          doLogout(logoutUrl);
          return false
        })
    ...
    function persistState(ttl) {
      if (isHTML5StorageSupported()) {
        sessionStorage.setItem(persistedStateKey, '{"ttl": ' + (new Date().getTime() + ttl) + ',"sessionData": ' + JSON.stringify(sessionData) + ', "address":"' + address + '"}');
        log("State persisted");
    ...
    function isPersistedStateValid(persistedData) {
      ...
      if (hashCookie === undefined && !isCurrentLocationPostCh()) {
        log("Cache validated because on a different host=[" + window.location.hostname + "] than control cookie domain=[" + controlCookieDomain + "]");
        return true
      }
      log("PersistedData are invalid [hashPersistedData=" + hashPersistedData + ",control cookie=" + hashCookie + "]");
      return false
    }
    ...
    function setControlCookie(scope, val) {
      ...
      switch (scope) {
        case 'hash':
          document.cookie = controlCookieName + "=" + val + ":" + getControlCookieVal('keepalive') + "; Path=/; domain=" + controlCookieDomain + sameSiteNoneSecure;
          break;
        case 'keepalive':
          document.cookie = controlCookieName + "=" + getControlCookieVal('hash') + ":" + val + "; Path=/; domain=" + controlCookieDomain + sameSiteNoneSecure;
          break;
        default:
          document.cookie = controlCookieName + "=" + val + "; Path=/; domain=" + controlCookieDomain + sameSiteNoneSecure;
          break
      }
    }
    ...
    function removeControlCookie() {
      if (retrySubscribeOnFail) {
        document.cookie = controlCookieName + "=" + "sub" + "; Path=/; domain=" + controlCookieDomain;
        log("Control cookie set to sub")
      } else {
        document.cookie = controlCookieName + "=" + "" + "; Path=/; Expires=Wed, 01 Apr 2014 01:00:00 GMT; domain=" + controlCookieDomain;
    ...
    function saveDocumentOnCache(document, documentType) {
      ...
      if (isHTML5StorageSupported()) {
        ...
        log("Document " + documentType + " has been persisted on cached with value " + JSON.stringify(document))
      }
    }
    ...
    function installKeepAliveTimerHandler() {
      if (conf.keepAlive && keepAliveTimer === undefined) {
        ...
        keepAliveTimer = window.setInterval(keepAliveTimerFunction, conf.keepAliveInterval * 60 * 1000)
      }
    }
    ...
    function keepAliveSessions() {
      if (isUserAuthenticated()) {
        var html = "<img src='" + platform.keepAliveURL + "/?" + random() + "'><img src='" + platformEndPoints.keepalive + "?" + random() + "'>";
        selectFromShadowDom().find("#" + keepAliveID).html(html);
    ...
    function keepAliveSessionsOnInit() {
      ...
      if (isNaN(lastKeepAlive) || now - parseInt(lastKeepAlive) > conf.keepAliveInterval * 60 * 1000) {
        log("Running keepAliveSessionsOnInit");
        keepAliveSessions();
        return
      }
      log("keepAliveSessionsOnInit not running due to [now=" + now + ",last=" + lastKeepAlive + ",interval=" + conf.keepAliveInterval * 60 * 1000)
    }
    ...
    function renderHiddenContainer(parentContainerSelector, containerId) {
      selectFromShadowDom().find(parentContainerSelector).append('<div id="' + containerId + '" style="display:none;"></div>')
    }

    function selectFromShadowDom() {
      return $(document.querySelector('swisspost-internet-header').shadowRoot.querySelector('post-klp-login-widget').shadowRoot.querySelector('.widget-wrapper'));
    }

    function renderAnonymousWidget() {
      selectFromShadowDom().find("#" + id).addClass("anonymous");
      selectFromShadowDom().find("#" + id).attr("data-custom-focus-id", "klp-widget")
      selectFromShadowDom().find('#' + id).html('<div class="klp-widget-anonymous"><div class="klp-widget-anonymous__wrapper">' + '<a tabindex="' + tabIndex('sign-in') + '" ' + accessKey('sign-in') + ' title="' + text('sign-in') + '" href="' + loginURL() + '" data-custom-focus-target="klp-widget" data-custom-focus-direction="parent" role="button"><span class="klp-widget-anonymous__text">' + text('sign-in') + '</span><svg class="ppm-svg-icon ppm-main-navigation__login-icon" focusable="false"><use xlink:href="#2064_arrow-enter"></use></svg></a>' + '</div></div>');
      selectFromShadowDom().find('#' + id).on('click touch', function (e) {
        e.preventDefault();
    ...
    function renderAuthenticatedWidget() {
      selectFromShadowDom().find('#' + id).off("click touch");
      var info = "", authenticatedSessionTailNameClass = "";
      ...
      selectFromShadowDom().find('#' + id).html('<div class="' + authenticatedSectionClass + '">' + '<div class="klp-widget-authenticated-session klp-widget-menu-close">' + '<a href="about:blank" role="button" class="klp-widget-authenticated-session-link klp-widget__user" title="' + text('title-text-menu') + '" tabindex="' + tabIndex('toggle-menu') + '" ' + accessKey('toggle-menu') + ' data-dropdown="klp-widget-authenticated-menu" aria-expanded="false" aria-controls="authenticated-menu">' + '<div class="' + authenticatedSessionTailNameClass + '">' + sessionData.name + '&nbsp;' + sessionData.surname + '</div>' + '<span class="initials-mobile">' + sessionData.name.substring(0, 1) + sessionData.surname.substring(0, 1) + '</span>' + '<span class="notification-number"></span>' + '</a>' + getAuthenticatedMenuLinks(authenticatedSessionTailNameClass, info, sessionData) + '</div>' + '</div>');
      ...
      selectFromShadowDom().find('#' + id + ' .klp-widget__user').on('click touch', function (e) {
        e.preventDefault();
        selectFromShadowDom().find('#' + id + ' .klp-widget__user').focus();
        toggleMenu();
        toggleNotificationsMenu();
        return false
      });
      selectFromShadowDom().find('#' + id + ' #klp-widget-authenticated-menu-logout').on('click touch', function (e) {
        e.preventDefault();
        selectFromShadowDom().find('#' + id + ' #klp-widget-authenticated-menu-logout').focus();
        doLogout(logoutURL());
        return false
      });
    ...
    function isOldChangeCompany() {
      return (sessionData.userType === 'B2B' && sessionData.canChangeCompany && sessionData.changeUserAndProfile == null) || (sessionData.changeUserAndProfile != null && sessionData.changeUserAndProfile === "profile")
    }

    function isChangeUserAndProfile() {
      return (sessionData.changeUserAndProfile != null && sessionData.changeUserAndProfile === "userAndProfile")
    }
    ...
    function getAuthenticatedMenuLinks(authenticatedSessionTailNameClass, info, sessionData) {
      ...
      if (menuLinks !== undefined) {
        ...
        menuList += '<li class="name-and-surname">' + '<div class="initials-circle">' + '<div class="initials-container">' + '<div>' + sessionData.name.substring(0, 1) + sessionData.surname.substring(0, 1) + '</div>' + '</div>' + '</div>' + '<div class="nameAndInfoWrapper"><span class="' + nameClass + '">' + sessionData.name + '&nbsp;' + sessionData.surname + '</span>' + '<span class="' + infoClass + '">' + info + '</span></div>' + '</li>';

        $.each(menuLinks, function (index, item) {
          menuList += '<li>' + '<a class="notification-link" title="' + item.description + '" href="' + item.url + '"><div class="' + item.iconclass + '"></div><div class="linkContainer">' + '<span class="klp-widget-notification-link-text">' + item.description + '</span></div></a>' + '</li>';
        });
        menuList += '<li>' + '<a class="notification-link" title="' + text('title-text-notifications') + '" href="' + messagesUrl + '"><div class="widget_icon_notification" aria-hidden></div><div class="linkContainer">' + '<span class="klp-widget-notification-link-text">' + text('title-text-notifications') + '</span><span class="notification-number-detail"></span></div></a>' + '</li>';
      }
      ...
      if (isOldChangeCompany()) {
        changeCompanyEntry = '<li>' + '<a id="klp-widget-authenticated-menu-changecompany" class="notification-link" tabindex="' + tabIndex('change-company') + '" ' + accessKey('change-company') + ' href="about:blank" role="button" title="' + text('change-company') + '"><div class="widget_icon_changecompany"></div><div class="linkContainer">' + '<span class="klp-widget-notification-link-text">' + text("change-company") + '</span></div></a>' + '</li>'
      } else if (isChangeUserAndProfile()) {
        changeCompanyEntry = '<li>' + '<a id="klp-widget-authenticated-menu-changecompany" class="notification-link" tabindex="' + tabIndex('change-account') + '" ' + accessKey('change-company') + ' href="about:blank" role="button" title="' + text('change-account') + '"><div class="widget_icon_changecompany"></div><div class="linkContainer">' + '<span class="klp-widget-notification-link-text">' + text("change-account") + '</span></div></a>' + '</li>'
      }
      return '<div class="klp-widget-authenticated-menu" id="authenticated-menu" data-dropdown-toggler="klp-widget__user">' + '<ul>' + menuList + changeCompanyEntry + '<li>' + '<a id="klp-widget-authenticated-menu-logout" class="notification-link" tabindex="' + tabIndex('sign-out') + '" ' + accessKey('sign-out') + ' title="' + text('sign-out') + '" href="about:blank" role="button"><div class="widget_icon_logout"></div><div class="linkContainer">' + '<span class="klp-widget-notification-link-text">' + text("sign-out") + '</span></div></a>' + '</li>' + '</ul>' + '</div>'
    }
    ...
    function renderNotificationsWidget(notifications) {
      if (notifications != null && $('.notification-number').text() !== notifications.unreadNotifications) {
    ...
    function toggleNotificationsMenu() {
      if (unreadNotifications !== 0) {
        if (unreadNotifications > 99) {
          selectFromShadowDom().find('.notification-number-detail').css('visibility', 'visible').text("99+");
        } else {
          selectFromShadowDom().find('.notification-number-detail').css('visibility', 'visible').text(unreadNotifications);
        }
      } else {
        selectFromShadowDom().find(".notification-number-detail").css('visibility', 'hidden');
      }
      ...
      if (!address) {
        if (trySubscription()) {
          ...
          fetch(platformEndPoints.subscribe, {
            ...
            credentials: 'include',
            mode: 'cors'
          }).then(message => message.json()).then(message => handleMessage(message)).catch((error) => {
    ...
    function openCommunication() {
      if (!eventBus) {
        ...
        eventBus.onclose = function () {
          log('EventBus closed');
          log("Communication closed with retrySubscribeOnFail=" + retrySubscribeOnFail + ". Retrying subscribe");
          eventBus = null;
    ...
    function toggleDropdown(dropdownToToggle) {
      ...
      closeDropdowns();
      selectFromShadowDom().find("." + dropdownToToggle.attr("data-dropdown-toggler")).attr('aria-expanded', true);
      selectFromShadowDom().find('#' + id).addClass("bubble");
      dropdownToToggle.show();
      $(document).on("click", closeDropdowns);
      if (dropdownToToggle.hasClass("klp-widget-authenticated-menu")) {
        dropdownToToggle.parent().removeClass("klp-widget-menu-close").addClass("klp-widget-menu-open")
      }
    }

    function closeDropdowns(e) {
      if (e != null) {
        if (selectFromShadowDom().find(e.target).parents('#' + id).length > 0) {
          return
        }
      }
      selectFromShadowDom().find(document).off("click", closeDropdowns);
      selectFromShadowDom().find('#' + id).removeClass("bubble");
      selectFromShadowDom().find('#' + id + ' .klp-widget-authenticated-menu').hide().parent().removeClass("klp-widget-menu-open").addClass("klp-widget-menu-close");
      selectFromShadowDom().find("." + selectFromShadowDom().find('#' + id + ' .klp-widget-authenticated-menu').attr("data-dropdown-toggler")).attr('aria-expanded', false);
    }

    function setArrowKeysListeners() {
      selectFromShadowDom().find('.klp-widget__user').on('keydown', function (event) {
        if (event.which < 37 || event.which > 40) {
          return
        }
        event.preventDefault();
        var parent = selectFromShadowDom().find(this).parent();
        var dropdown = selectFromShadowDom().find("." + selectFromShadowDom().find(this).attr("data-dropdown"));
        switch (event.which) {
          case 37:
      ...
      selectFromShadowDom().find('.klp-widget-authenticated-menu').on('keydown', 'a', function (event) {
        if (event.which < 37 || event.which > 40) {
          return
        }
        event.preventDefault();
        var parent = selectFromShadowDom().find(this).parent();
        var dropdownToggler = selectFromShadowDom().find("." + selectFromShadowDom().find(this).parents("div").first().attr("data-dropdown-toggler"));
        switch (event.which) {
          case 37:
          case 38:
