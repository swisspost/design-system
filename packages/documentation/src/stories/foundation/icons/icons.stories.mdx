...
Icons have a default `width` of `1em`, which means they will scale with the size of the current `font-size`. Therefore you can use our [font-size utility](/?path=/docs/utilities-text--font-size) to manage the size of an icon. Or you can use custom CSS to set the icon size, either via direct style attribute, or via custom classes.
