...
Icons have a default `width` of `1em`, which means they will scale with the size of the current `font-size`. Therefore, you can manage the size of an icon using our [font-size utilities](/?path=/docs/utilities-text--font-size) or by setting the CSS directly through a style attribute or self-defined classes.
