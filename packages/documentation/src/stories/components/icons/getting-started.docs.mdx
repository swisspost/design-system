...
make sure that `default-src` is set to `'self'`. When using the Post-Icon component, icons are retrieved from [`https://unpkg.com/`](https://unpkg.com/) therefore this URL should be added to the `connect-src` in index.html.
