...
To enhance individual accordion items with a logo, add either an `<img>` or a `<post-icon>` to the logo slot.
This will display the logo or icon in front of the accordion item's heading.

**Image Logo:**
Use an `<img>` element with the `logo` slot to display a custom image. The image will be scaled to fit the allocated space.

**Icon Logo:**
Use a `<post-icon>` element with the `logo` slot to display a Swiss Post Design System icon. Icons are responsive and scale automatically:
