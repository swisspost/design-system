...
| Key | Behaviour |
|---|---|
| `Down Arrow` | Opens the listbox or moves to the next option. Wraps to the first option at the end. |
| `Up Arrow` | Opens the listbox or moves to the previous option. Wraps to the last option at the start. |
| `Alt + Down Arrow` | Opens the listbox without changing the active option. |
| `Enter` | Selects the currently highlighted option. |
| `Escape` | Closes the listbox. |
| `Tab` | Accepts the current selection and moves focus to the next form element. |
| `Home` / `End` | Moves to the first or last visible option. |
| `Space` | Opens the listbox if closed. |
