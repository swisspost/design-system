...
Every change on the screen should be left for at least 3-6 seconds (for a two-liner) so that the user can perceive it.
...
Last but not least, if the user moves the mouse or the focus to the `.toast` element, the automatic closing should be interrupted/paused. If the mouse/focus leaves the element again, the timeout for the automatic closing can be restarted.
