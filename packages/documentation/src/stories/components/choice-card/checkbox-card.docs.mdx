...
<div className="alert alert-info">
  ...
  of classes (see below for a snippet):
  <ul>
    <li>
      <code>checked</code>
    </li>
...
<Source
  code={`
['focusin', 'focusout', 'input'].forEach(t => document.addEventListener(t, e => {
  if (!(e.target instanceof Element) || e.target.nodeName !== 'input') return; 
  const parent = e.target.parentElement;
  if (!parent?.classList.contains('checkbox-button-card')) return;
  switch (e.type) {
    case 'focusin':
