...
export default {
  ...
  decorators: [
    (Story: Story) => <div onClick={ e => e.preventDefault() }>
      <Story/>
    </div>
  ]
} as Meta;

const StoryDecoratorGird = (Story: Story) => <div className="row">
...
const Template = (args: Args) => {
  ...
  const image = <img className={ imageClasses } src="https://picsum.photos/id/20/300/200" alt="Card image"/>;
  ...
  const header = args.showHeader ? args.customHeader ?? <div className="card-header">Kapo Titulum</div> : null;

  const title = args.showTitle ? <h5 className="card-title">Titulum</h5> : null;
  const subtitle = args.showSubtitle ? <h6 className="card-subtitle mb-2 text-muted">Sub Titulum</h6> : null;
  const links = args.showLinks ? [<a key="link1" href="#" className="card-link">Ligilo teksto</a>, <a key="link2" href="#" className="card-link">Pli da ligo</a>] : null;
  const button = args.showButton ? <button className="btn btn-primary btn-animated"><span>Butonon teksto</span></button> : null;
  const body = args.showBody ? args.customBody ?? <div className="card-body">
    { title }
    { subtitle }
  ...
  const listGroup = args.showListGroup ? <ul className="list-group">
    <li className="list-group-item">Ero</li>
    <li className="list-group-item">Dua ero</li>
    <li className="list-group-item">Alio ero</li>
  </ul> : null;

  const footer = args.showFooter ? args.customFooter ?? <div className="card-footer">Piedo Contentus momentus</div> : null;
...
CustomContent.args = {
  showImage: false,
  showHeader: true,
  customHeader: <div className="card-header d-flex">
  ...
  showBody: true,
  customBody: <ul className="list-group">
    <li className="d-block list-group-item">
      <div className="d-flex justify-content-between">
        <div>Mr<br/>
          Pronomo Familinomo<br/>
          Strato 1<br/>
          1234 Urbo
        </div>
        <div className="align-self-center">
          <a href="#"><span className="pi pi-3193"></span></a>
        </div>
      </div>
    </li>
    <li className="d-block list-group-item">
      <div className="d-flex justify-content-between">
        <div>Lingvo: <span className="fw-bold">Germana</span></div>
        <div className="align-self-center">
          <a href="#"><span className="pi pi-3193"></span></a>
        </div>
      </div>
  ...
  showButton: false,
  showFooter: true,
  customFooter: <div className="card-footer card-links">
    <a href="#">Aldonu adreson</a><a href="#">Konfirmu numeron</a>
  </div>,
};
...
ImageOverlay.decorators = [
  (Story: Story) => <div className="row" onClick={ e => e.preventDefault() }>
