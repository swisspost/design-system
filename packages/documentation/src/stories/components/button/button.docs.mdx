...
and use the `flex-row-reverse`, `flex-column-reverse` classes or the `order` CSS property to change the visual order. In the following example, the send button appears first on tab navigation.
