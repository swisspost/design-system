...
To generate a list of items with unique tooltips, use Angular's `@for` to create triggers dynamically.
Loop through your elements to render each `<post-tooltip-trigger>`, and use a single `<post-tooltip>` component whose content updates based on the active trigger.
You can achieve this by listening to the custom `postTriggered` event – emited on hover – and setting the active item accordingly.
