...
export const CardControl: StoryObj = {
  render: (_args: Args, context: StoryContext) => {
      ...
      () => html`
        ${PALETTE_TEST_COMPONENT_TYPES.map(
          type =>
            html`<div>
              <p>Type: ${type}</p>
              <div class="row g-0 my-8">${renderBombedArgs(type)}</div>
              <div class="row g-0">
                <div class="col-sm-4 py-8">
                  <fieldset class="mx-0 px-8">
                    <legend>Body</legend>
                ...
                ${PALETTE_TEST_PALETTE_TYPES.map(
                  palette =>
                    html`<div class="col-sm-4 py-8 palette palette-${palette}">
                      <fieldset class="mx-0 px-8">
    ...
    function renderBombedArgs(type: string) {
      return bombedArgs.map(
        args =>
          html`<div class="col-sm-3">
            <fieldset class="m-0 px-8">${meta.render?.({ ...args, type }, context)}</fieldset>
          </div>`,
      );
    }
  },
