...
#### With an empty default option

If you use an empty option in your select and you want that your label acts as placeholder,
you can add dynamically the class `.form-select-empty` to the `<select>` when there are no value defined or no option selected.

<Canvas of={SelectStories.FloatingLabelNoSelectedOption} />
