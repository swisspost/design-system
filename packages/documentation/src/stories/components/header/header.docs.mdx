...
<p><b>The `.selected` class highlights the active navigation item.</b> 
It should be applied by application logic to the `<a>` or `<button>` element inside the active `<post-list-item>` of the `<post-mainnavigation>`, as shown in the example below.</p>
