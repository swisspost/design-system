...
**A `.selected` class must be applied by your application logic to indicate the active page.**

This class is not added automatically;
it must be applied directly to the `<a>` element inside `<post-mainnavigation>` that represents the page currently being viewed.
Once applied, the parent items in the hierarchy automatically inherit the correct selected styling.
