...
<p className="lead">Avatar picture is used to display a user profile as a picture or the user initials.</p>

The `<post-avatar-picture>` component provides several properties to change its appearance. The most important one is probably the `email` property, which is used to request a Gravatar picture.
