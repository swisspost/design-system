...
To make it responsive, you can add `.row-cols-*` classes to the row (see [Grid page](/?path=/docs/layout-grid--docs#row-columns)).

<Canvas of={FormPatternsStories.RowSimple} sourceState="shown" />

For finer adjustment, you can also use the `.col-*` classes (see [Columns page](/?path=/docs/layout-columns--docs)) on your columns. For example for a 2/1 ratio, given that a row has twelve columns in total,
