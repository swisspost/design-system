...
`.gx-{size}` classes can be used to set the horizontal gutters within the columns (`.col`) of a `.row`.
...
`.gy-{size}` classes can be used to set the vertical gutter width when a `.row` wraps to new lines.
...
`.g-{size}` classes can be used to set both the horizontal and vertical gutter widths.
...
The gutters can be removed with the use of the following classes: `.g-0`, `gx-0` and `gy-0`.
