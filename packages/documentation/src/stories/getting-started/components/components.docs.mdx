...
Import all the available components lazy-loaded into your project (for example into your `/src/main.ts` or `/src/index.js` file) and make them work:

<Source code={SampleUseAll} language="javascript" />

Import only a few specific components and handle bundling and lazy-loading by yourself:

<Source code={SampleUseSpecific} language="javascript" />

Use the imported components in your html:

<Source code={`<post-icon name="1000"></post-icon>`} language="html" />
