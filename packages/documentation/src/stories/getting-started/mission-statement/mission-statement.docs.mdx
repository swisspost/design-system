...
<ul className="list--roles d-flex m-0 p-0">
  {roles.map((role, index) => (
    <li key={index} className="list--roles__item mb-3 p-3 h-100">
      <strong>{role.role}</strong>
      <div className="mt-2">{role.description}</div>
    </li>
  ))}
</ul>
...
<ul className="list-group">
  {stakeholders.map((stakeholder, index) => (
    <li key={index} className="list-group-item w-sm-100 w-md-50 w-lg-25">{stakeholder.role}</li>
  ))}
</ul>
...
<ul className="list-group list-group-horizontal-md mb-3 p-0 justify-content-center-lg align-items-center-lg">
  {teamMembers.map((teamMember, index) => (
    <li key={index} className="list-group-item py-4 gap-3 fw-bold display-flex flex-lg-column w-md-25 justify-content-center">
...
<p className="mt-4 mb-1 text-align-start">
  The quality of the Design System is being measured by pre-defined KPIs and is constantly improved.
</p>

<div className="progress-system-quality">
  <div className="progress-system-quality__bar"></div>
</div>
