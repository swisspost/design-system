...
<p class="alert alert-info alert-md">
  ...
  render the icons (which we strongly recommend), you must also install the <strong>components</strong>{' '}
  package.
  <br />
...
Just open your `anuglar.json` file and add the following to the `build.options.assets`:
