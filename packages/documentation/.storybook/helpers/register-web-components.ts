...
import {
  StencilJsonDocs, StencilJsonDocsMethod,
  StencilJsonDocsProp,
  StencilJsonDocsTag,
} from '@kurbar/storybook-addon-docs-stencil/dist/types';
...
if (postComponentsDocJson && internetHeaderDocJson) {
  ...
  const addDeprecationAlert = (tag: StencilJsonDocsProp | StencilJsonDocsMethod) => {
    const deprecationAlert = `<span className="mb-micro alert alert-warning alert-sm">**Deprecated:** ${tag.deprecation}</span>`;
    tag.docs = `${tag.deprecation ? deprecationAlert : ''}${tag.docs}`;
  }
  components.forEach(component => {
    component.props.forEach(addDeprecationAlert);
    component.methods.forEach(addDeprecationAlert);
  });
