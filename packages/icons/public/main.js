...
document.addEventListener('alpine:init', () => {
  Alpine.data('TEST_ICONS', () => ({
    ...
    getTestIcon(name) {
      return this.sizes.map(
        size =>
          `<use href="/icons/${name}-sprite.svg?v=${new Date().getTime()}#${name}-${size}" style="display: var(--post-icon-size-${size}, none)"/>`,
      );
    },
  }));
