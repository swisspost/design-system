...
| `textExpandHome` _(required)_  | `text-expand-home` | An accessible label for the overflow menu that contains the home item, shown only once every middle item has been collapsed and there is still no room left for the home item. | `string`  | `undefined` |
