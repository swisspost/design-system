...
| `textExpandHome` _(required)_  | `text-expand-home` | An accessible label for the overflow menu that contains the home item, shown only once there is no room left for the home item even after every middle item has been collapsed and the last (selected) item has wrapped onto multiple lines. | `string`  | `undefined` |
