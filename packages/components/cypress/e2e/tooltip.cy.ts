describe('tooltips', { baseUrl: null }, () => {
  beforeEach(() => {
    ...
    cy.get('#t1').shadow().get('p[popover]').as('tooltip1');
    cy.get('#t2').shadow().get('p[popover]').as('tooltip2');
  });
  ...
  describe('default', () => {
    ...
    it('tooltip placement right', () => {
      cy.get('@tooltip2').invoke('attr', 'placement', 'right');
      cy.get('@target2').focus();
      cy.get('@tooltip2')
        .should('have.css', 'left')
        .then((v: any) => {
          expect(parseInt(v)).to.be.greaterThan(500);
        });
    });
  });
