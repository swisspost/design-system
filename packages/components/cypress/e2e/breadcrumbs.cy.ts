...
describe('breadcrumbs', () => {
  ...
  describe('home link', () => {
    ...
    describe('home text-only', () => {
      beforeEach(() => {
        cy.getComponent('breadcrumbs', BREADCRUMBS_ID, 'home-text-only');
        cy.get('post-breadcrumbs[data-hydrated]', { timeout: 30000 }).as('breadcrumbs');
      });

      it('should hide the home icon and display text-home visibly when home-text-only is true', () => {
      ...
      it('should restore the default home icon when home-text-only is set back to false', () => {
        cy.viewport(2000, 400);
        cy.get('@breadcrumbs').invoke('removeAttr', 'home-text-only');
