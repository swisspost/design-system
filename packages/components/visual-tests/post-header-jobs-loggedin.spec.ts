...
async function openUserMenu(page: Page): Promise<void> {
  await page.evaluate(() => {
    const userMenu = document.querySelector('post-user-menu');
    const trigger = userMenu?.querySelector('post-menu-trigger');
    const button = trigger?.querySelector('button');
    button?.click();
  });
  await page.waitForTimeout(WAIT_TIMES.animation);
...
async function hoverUserMenuTrigger(page: Page): Promise<void> {
  ...
  const coords = await page.evaluate(() => {
    const userMenu = document.querySelector('post-user-menu');
    const trigger = userMenu?.querySelector('post-menu-trigger button');
    if (trigger) {
      const rect = trigger.getBoundingClientRect();
      return { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
    }
...
async function hoverUserMenuItem(page: Page, itemIndex: number): Promise<void> {
  await page.evaluate(() => (document.activeElement as HTMLElement)?.blur());
  await page.waitForTimeout(100);

  const coords = await page.evaluate((index) => {
    const userMenu = document.querySelector('post-user-menu');
    const items = userMenu?.querySelectorAll('post-menu-item');
    const item = items?.[index] as HTMLElement;
    if (item) {
      const rect = item.getBoundingClientRect();
      return { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
...
async function focusUserMenuItem(page: Page, itemIndex: number): Promise<void> {
  await page.evaluate((index) => {
    const userMenu = document.querySelector('post-user-menu');
    const items = userMenu?.querySelectorAll('post-menu-item');
    const item = items?.[index] as HTMLElement;
