...
<div className="lead">
  The radio button lets users select one option from a group of options.
</div>

Our component customizes the native <code>&lt;input type="radio"&gt;</code> with CSS that overrides its default appearance, staying
consistent across browsers and devices. Custom icons indicate the checked and indeterminate states.
