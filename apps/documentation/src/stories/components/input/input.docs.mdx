...
<p className="lead">The input lets users enter text into a UI.</p>

Our component customizes the native `<input>` with CSS. Add the `.form-control` class to trigger the custom styles.
...
For a more compact version of the input, add the `.form-control-sm` class to it. This variant only works with non-floating labels.
