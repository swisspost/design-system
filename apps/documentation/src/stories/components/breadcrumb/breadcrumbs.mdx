...
<post-banner type="info">
  The `aria-current="page"` attribute is not set automatically on a slotted `<a>`. Add it yourself alongside `selected`, which still applies the correct visual styling and controls the item's position in the breadcrumb trail.
</post-banner>
