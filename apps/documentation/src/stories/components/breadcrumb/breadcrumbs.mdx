...
By default the root breadcrumb shows a home icon, with `text-home` used only as its accessible label. Set `show-home-text` to `true` to show `text-home` visibly instead — useful when the root should link to a segment's own home page (e.g. "Private customers", "About us") rather than the generic site root.
