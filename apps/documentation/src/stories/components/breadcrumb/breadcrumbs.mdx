...
<post-banner type="info">
  When slotting your own `<a>`, the `url`/`home-url`, `label`, and `description` properties are ignored, and `aria-current="page"` is not set automatically. Set these directly on the slotted `<a>` instead. `selected` still applies the correct visual styling and controls the item's position in the breadcrumb trail.
</post-banner>
