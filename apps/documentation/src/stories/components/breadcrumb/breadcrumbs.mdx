...
By default, the root (home) breadcrumb item displays a home icon, with `text-home` used only as its accessible label. Set `home-text` to `true` to display `text-home` visibly instead of the icon, enabling segment specific breadcrumbs that link to a segment's own home page (like "Private customers" or "About us") rather than the generic site root.
