...
When the trigger and collapsible are co-located, you can nest the `<post-collapsible>` directly inside `<post-collapsible-trigger>` and omit the `for` attribute and the `id` entirely.

<Canvas of={CollapsibleStories.Wrapped} />
