...
The Header for Internal applications is a consistently available interface element that provides a consistent navigation experience across the whole application.

<Canvas of={HeaderStories.InternalApplications} inline="false" />

#### Example with Language Menu

In the Internal Applications header, if a language menu is required, it should be placed in the local navigation `slot="local-nav"`.

<Canvas of={HeaderStories.InternalApplicationsWithLanguageMenu} inline="false" />
