...
The `language` and `active-route` props are still reactive and can be set at runtime.
The `environment` and `full-width` props are accepted but do not trigger reactive updates after initial render.
