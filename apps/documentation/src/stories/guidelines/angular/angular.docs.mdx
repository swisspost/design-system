...
import { getDocsLinkForPackage, getHumanReadableVersionForPackage } from '@/utils/package';
...
<ul className="tile-wrapper" role="list">
  {Object.values(packages)
    .filter(pkg => pkg.type === PackageType.Wc)
    .map(pkg => (
      <li key={pkg.name}>
        <ti-le title={pkg.name} href={getDocsLinkForPackage(pkg)} ariaLabel={pkg.docs.linkAriaLabel}>
...
Even though Angular supports Web components technologies natively, they are less convenient to use than regular Angular components.
    ...
    <tr>
      <td>
        <ul>
          <li>Type-checking</li>
          <li>
            Angular component wrappers will be detached from change detection, preventing
            unnecessary repaints of your web component
          </li>
          <li>
          ...
          <li>
            Optionally, form control web components can be used as control value accessors with
            Angular's reactive forms or `[ngModel]`.
          </li>
          <li>
            It is not necessary to include the Angular `CUSTOM_ELEMENTS_SCHEMA` in all modules
            consuming your Stencil components.
          </li>
        </ul>
      </td>
      <td>
