...
import * as FormGuidelinesStories from './forms.stories';

<Meta of={FormGuidelinesStories} />
...
<Canvas of={FormGuidelinesStories.BasicInput} sourceState="shown" />

## Vertical spacing

To add space between input, you can use [margin utilities](/?path=/docs/facaacfd-18f1-49b4-80f1-a96680730fa0--docs).

<Canvas of={FormGuidelinesStories.VerticalSpacing} sourceState="shown" />
...
<Canvas of={FormGuidelinesStories.RowSimple} sourceState="shown" />
...
<Canvas of={FormGuidelinesStories.ColSimple} sourceState="shown" />
...
<Canvas of={FormGuidelinesStories.CustomWidth} sourceState="shown" />
...
<Canvas of={FormGuidelinesStories.Buttons} sourceState="shown" />
...
<Canvas of={FormGuidelinesStories.FloatingLabels} sourceState="shown" />
...
- To have the message displayed, one of those two options are available:
  1. The form needs the class `.was-validated`. With this class you can manage when to display errors. Usually, errors should not be
     shown on untouched fields.
  2. The input field needs the class `.is-invalid`/`.is-valid`

**Accessibility**

`aria-describedby` is used to associate a form control to a validation message or a help text, by referencing the `id` of the corresponding element. When a field is invalid, `aria-invalid="true"` should be applied to indicate its state to assistive technologies.

More informations about this can be found in our [Form validation documentation](/?path=/docs/1aa900d9-aa65-4ae0-b8cd-e6cca6cc3472--docs).

<Canvas of={FormGuidelinesStories.Validation} sourceState="hidden" />

## Required and optional fields

Mark all required fields with the label “required” and all optional fields with the label “optional”. However, not specifying that a field is optional is not a deal breaker, but doing so is a nice perk.

<Canvas of={FormGuidelinesStories.RequiredOptional} sourceState="hidden" />
...
<Canvas of={FormGuidelinesStories.Hints} sourceState="shown" />
...
<Canvas of={FormGuidelinesStories.Footer} sourceState="shown" />
