...
<ul className="team-list no-style-list row">
  {teamMembers.map((teamMember, index) => (
    <li
      key={index}
      className="team-item list-item centered-item col-sm-12 col-md-4 col-lg-3 flex-lg-column"
    >
      <post-icon
        name={teamMember.icon}
        style={{ fontSize: '3rem' }}
        className="team-icon"
      ></post-icon>
      {teamMember.name}
...
<post-banner>Onboarding is mandatory for external agencies.</post-banner>
